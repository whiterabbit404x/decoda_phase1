...
test('localhost:3000 renders the Feature 4 dashboard section without a fatal crash', async ({ page }) => {
  ...
  await expect(page.locator('h2', { hasText: 'Feature 4 · Interoperability & Systemic Resilience' })).toBeVisible();
  await expect(page.locator('text=Feature 4 demo interactions')).toBeVisible();
